...
class Soft_CrossEn(nn.Module):
    def __init__(self, ):
        super(Soft_CrossEn, self).__init__()

    def forward(self, sim_matrix, soft_label, intra_sim):
        B, _ = sim_matrix.size()
        hard_label = torch.eye(B, device=soft_label.device)
        label = (1 - intra_sim) * hard_label + intra_sim * F.softmax(soft_label, dim=-1)
        logpt = F.log_softmax(sim_matrix, dim=-1)
        nce_loss = -logpt * label
        sim_loss = nce_loss.sum() / B
        return sim_loss
