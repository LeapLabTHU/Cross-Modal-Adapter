...
from .dataloader_retrieval_images import RetrievalDataset
...
class MSRVTTDataset(RetrievalDataset):
    ...
    def __init__(self, subset, anno_path, video_path, tokenizer, max_words=32,
                 max_frames=12, video_framerate=1, image_resolution=224, slice_framepos=2, mode='all', config=None):
        super(MSRVTTDataset, self).__init__(subset, anno_path, video_path, tokenizer, max_words,
                                            max_frames, video_framerate, image_resolution, slice_framepos, mode, config=config)
        pass

    def _get_anns(self, subset='train'):
        """
        image_dict: dict: video_id -> image_folder_path
        ...
        self.sentences = defaultdict(list)  # saving full sentences anns
        image_dict = OrderedDict()
        sentences_dict = OrderedDict()  # saving one sentence ann
        ...
        for itm in data['sentences']:
            if itm['video_id'] in video_id_list:
                image_dict[itm['video_id']] = join(self.video_path, "{}.mp4".format(itm['video_id']))
                if self.config.expand_msrvtt_sentences and subset == 'train':
        ...
        return image_dict, sentences_dict
